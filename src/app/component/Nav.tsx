...
export default function Navbar() {
  ...
  return (
    <ProtectedRoute>
      <div className="flex items-center justify-end gap-1 px-2 py-2 bg-white sm:dark:bg-[var(--color-childbgdark)] max-sm:bg-transparent">

        {/* ── Theme toggle ─────────────────────────────────────────────── */}
        <button
          onClick={toggleTheme}
          className="relative w-8 h-8 rounded-xl flex items-center justify-center transition-all duration-150 bg-gray-100 dark:bg-white/[0.06] hover:bg-gray-200 dark:hover:bg-white/10 max-sm:bg-white/10 max-sm:hover:bg-white/20"
          title="Toggle theme"
        >
          {dark
            ? <Sun size={15} className="text-amber-400" />
            : <Moon size={15} className="dark:text-gray-500 text-gray-800 max-sm:dark:text-white" />
          }
        </button>
        ...
        <nav style={{ zIndex: 1000 }}>
          <ul className="flex items-center gap-1">
            ...
            <li ref={notificationsRef} className="relative max-md:hidden">
              <button
                className="relative w-8 h-8 rounded-xl flex items-center justify-center bg-gray-100 dark:bg-white/[0.06] hover:bg-gray-200 dark:hover:bg-white/10 transition-all duration-150"
              ...
              <div className={`${dropdownBase} top-full right-0 mt-2 origin-top-right ${openMenu === "notifications" ? dropdownVisible : dropdownHidden}`}>
                <div className="w-[300px] rounded-2xl overflow-hidden bg-white dark:bg-[#0f1117] border border-gray-100 dark:border-white/[0.08] shadow-2xl">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-white/[0.06]">
                    <div className="flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" />
                      <span className="text-xs font-black uppercase tracking-widest text-gray-500 dark:text-gray-400">Notifications</span>
                    </div>
                  ...
                  <div className="flex flex-col items-center justify-center py-10 gap-2">
                    <div className="w-10 h-10 rounded-2xl bg-gray-50 dark:bg-white/[0.04] flex items-center justify-center">
                      <Bell size={16} className="text-gray-300 dark:text-gray-600" />
                    </div>
                    <p className="text-xs text-gray-400 dark:text-gray-600 font-medium">No new notifications</p>
                  </div>
                </div>
              </div>
            </li>
            ...
            <li ref={quickAddRef} className="relative max-md:hidden">
              ...
              <div className={`${dropdownBase} top-full right-0 mt-2 origin-top-right ${openMenu === "quickAdd" ? dropdownVisible : dropdownHidden}`}>
                <div className="w-[220px] rounded-2xl overflow-hidden bg-white dark:bg-[#0f1117] border border-gray-100 dark:border-white/[0.08] shadow-2xl max-h-[calc(100vh-80px)] overflow-y-auto [&::-webkit-scrollbar]:hidden">
                  ...
                  <div className="py-2">
                    {quickadds.map((item, i) => (
                      <Link key={item.name + i} href={item.link} onClick={() => setOpenMenu(null)}
                        className="group flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-white/[0.04] transition-colors duration-100"
                      >
                        <span className="w-5 h-5 rounded-md flex items-center justify-center shrink-0"
                          style={{ backgroundColor: "color-mix(in srgb, var(--color-primary) 12%, transparent)" }}>
                          <Plus size={10} style={{ color: "var(--color-primary)" }} />
                        </span>
                        <p className="text-xs font-medium text-gray-600 dark:text-gray-400 whitespace-nowrap group-hover:text-gray-900 dark:group-hover:text-white transition-colors">
                          {item.name}
                        </p>
                      </Link>
                    ))}
            ...
            */}
            <li ref={adminMailRef} className="relative">
              <button
                className="flex items-center gap-2 h-8 px-2 rounded-xl hover:bg-gray-100 dark:hover:bg-white/[0.06] max-sm:hover:bg-white/10 transition-all duration-150"
              ...
              >
                ...
                <span className="text-xs font-semibold text-gray-700 dark:text-gray-300 max-sm:text-white max-md:hidden">
            ...
            <li className="max-md:hidden">
              <button
                ...
                className="w-8 h-8 rounded-xl flex items-center justify-center bg-gray-100 dark:bg-white/[0.06] hover:bg-red-50 dark:hover:bg-red-500/10 text-gray-400 hover:text-red-500 transition-all duration-150"
                title="Logout"
              >
                <LogOut size={14} />
              </button>
...
function AdminDropdownPanel({
  ...
  return (
    <div className="w-[210px] rounded-2xl overflow-hidden bg-white dark:bg-[#0f1117] border border-gray-100 dark:border-white/[0.08] shadow-2xl shadow-black/20">
      {/* Top accent line */}
      <div className="h-[3px]" style={{ background: "linear-gradient(90deg, var(--color-primary), transparent)" }} />
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-2.5 transition-colors duration-100 ${
              item.danger
                ...
                : "hover:bg-gray-50 dark:hover:bg-white/[0.04] text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white"
            }`}
          >
            <span className={item.danger ? "text-red-400" : "text-gray-400 dark:text-gray-500"}>
              {item.icon}
            </span>
