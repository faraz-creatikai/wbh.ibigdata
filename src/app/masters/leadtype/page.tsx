...
import { leadtypeDialogDataInterface, leadtypeGetDataInterface } from "@/store/masters/leadtype/leadtype.interface";
import { deleteLeadType, getLeadType } from "@/store/masters/leadtype/leadtype";

export default function LeadTypePage() {
    const [leadtype, setLeadType] = useState<leadtypeGetDataInterface[]>([]);
    const [keyword, setKeyword] = useState("");
    const [limit, setLimit] = useState("10");
    const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
    const [deleteDialogData, setDeleteDialogData] = useState<leadtypeDialogDataInterface | null>(null);
    ...
    const fetchLeadType = async () => {
        const data = await getLeadType();
        if (data) {
            const formattedData = data.map((r: leadtypeGetDataInterface) => ({
                ...r,
                Name: r.Name.charAt(0).toUpperCase() + r.Name.slice(1)
            }));
            setLeadType(formattedData);
        }
    };
    ...
    // Filtered leadtype
    const filteredLeadType = useMemo(() => {
        return leadtype
            .filter((r) => keyword === "" || r.Name.toLowerCase().includes(keyword.toLowerCase()))
    }, [leadtype, keyword, limit]);

    // Delete leadtype
    const handleDelete = async (data: leadtypeDialogDataInterface | null) => {
        if (!data) return;
        const res = await deleteLeadType(data.id);
        if (res) {
            toast.success("LeadType deleted successfully!");
            setIsDeleteDialogOpen(false);
            setDeleteDialogData(null);
            fetchLeadType();
            return;
        }
        toast.error("Failed to delete leadtype.");
    };

    // Edit leadtype
    const handleEdit = (id?: string) => {
        router.push(`/masters/leadtype/edit/${id}`);
    };
    ...
    const totalTablePages = Math.ceil(filteredLeadType.length / rowsPerTablePage);
    const indexOfLastRow = currentTablePage * rowsPerTablePage;
    const indexOfFirstRow = indexOfLastRow - rowsPerTablePage;
    const currentRows = filteredLeadType.slice(indexOfFirstRow, indexOfLastRow);
    ...
    return (
        <MasterProtectedRoute>
            ...
            <div className="min-h-[calc(100vh-56px)] overflow-auto max-md:py-10">
                ...
                {/* Delete Dialog */}
                <DeleteDialog<leadtypeDialogDataInterface>
                    isOpen={isDeleteDialogOpen}
                    title="Are you sure you want to delete this leadtype?"
                ...
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 relative">
                    <PageHeader title="Dashboard" subtitles={["LeadType"]} />
                    {/* Add Button */}

                    <AddButton
                        url="/masters/leadtype/add"
                        text="Add"
                    ...
                    <form className="w-full flex flex-wrap gap-6 items-end mb-6 mt-16">
                        ...
                        <div className="flex flex-col flex-1 w-60">
                            ...
                            <input
                                id="keyword"
                                type="text"
                                placeholder="Search by leadtype name..."
                                value={keyword}
