...
import { addLeadType } from "@/store/masters/leadtype/leadtype";

interface ErrorInterface {
  [key: string]: string;
}

export default function LeadTypeAdd() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await addLeadType(leadtypeData);
      if (result) {
        toast.success("LeadType added successfully!");
        router.push("/masters/leadtype");
      }
    } catch (error) {
      toast.error("Failed to add leadtype");
      console.error("LeadType Add Error:", error);
    }
  };
            ...
            <form onSubmit={(e) => e.preventDefault()} className="w-full">
              <div className="mb-8 text-left border-b pb-4 border-gray-200">
                <h1 className="text-3xl font-extrabold text-[var(--color-secondary-darker)] leading-tight tracking-tight">
                  Add <span className="text-[var(--color-primary)]">LeadType</span>
                </h1>
              </div>
