...
export default function FunctionalAreaAdd() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await addFunctionalArea(functionalAreaData);
      if (result) {
        toast.success("Functional Area added successfully!");
        router.push("/masters/functional-areas");
      }
    } catch (error) {
      toast.error("Failed to add functional area");
      console.error("Functional Area Add Error:", error);
    }
  };
          ...
          <form onSubmit={(e) => e.preventDefault()} className="w-full">
            <div className="mb-8 text-left border-b pb-4 border-gray-200">
              <h1 className="text-3xl font-extrabold text-[var(--color-secondary-darker)] leading-tight tracking-tight">
                Add <span className="text-[var(--color-primary)]">Functional Area</span>
              </h1>
            </div>
