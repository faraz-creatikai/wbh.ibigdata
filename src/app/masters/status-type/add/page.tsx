...
import { statustypeAllDataInterface } from "@/store/masters/statustype/statustype.interface";
...
export default function StatusTypeAdd() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await addStatusType(statusTypeData);
      if (result) {
        toast.success("Status Type added successfully!");
        router.push("/masters/status-type");
      }
    } catch (error) {
      toast.error("Failed to add status type");
      console.error("StatusType Add Error:", error);
    }
  };
          ...
          <form onSubmit={(e) => e.preventDefault()} className="w-full">
            <div className="mb-8 text-left border-b pb-4 border-gray-200">
              <h1 className="text-3xl font-extrabold text-[var(--color-secondary-darker)] leading-tight tracking-tight">
                Add <span className="text-[var(--color-primary)]">Status Type</span>
              </h1>
            </div>
