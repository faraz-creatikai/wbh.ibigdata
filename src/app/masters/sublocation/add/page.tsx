...
import { subLocationAllDataInterface } from "@/store/masters/sublocation/sublocation.interface";
import { addsubLocation } from "@/store/masters/sublocation/sublocation";
import { getCity } from "@/store/masters/city/city";
import { getLocationByCity } from "@/store/masters/location/location"; // 👈 MUST EXIST
...
export default function SubLocationAdd() {
  const router = useRouter();

  const [subLocationData, setSubLocationData] =
    useState<subLocationAllDataInterface>({
      Name: "",
      Status: "",
      City: "",
      Location: "",
  ...
  const validateForm = () => {
    ...
    if (!subLocationData.Name.trim())
      newErrors.Name = "SubLocation Name is required";
    if (!subLocationData.City)
      newErrors.City = "City is required";
    if (!subLocationData.Location)
      newErrors.Location = "Location is required";
    if (!subLocationData.Status)
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await addsubLocation(subLocationData);
      if (result) {
        toast.success("SubLocation added successfully!");
        router.push("/masters/sublocation");
      }
    } catch {
      toast.error("Failed to add SubLocation");
    }
  };
            ...
            <form onSubmit={(e) => e.preventDefault()}>
              <div className="mb-8 border-b pb-4">
                <h1 className="text-3xl font-extrabold">
                  Add <span className="text-[var(--color-primary)]">SubLocation</span>
                </h1>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6 max-lg:grid-cols-1">
                ...
                <SingleSelect
                  ...
                  onChange={(v) =>
                    setSubLocationData({ ...subLocationData, Status: v })
                  }
                />
              </div>
