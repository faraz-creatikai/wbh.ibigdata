...
import {
    referencesAllDataInterface,
    referencesGetDataInterface,
    referencesDialogDataInterface
} from "@/store/masters/references/references.interface";
import {
    deleteReferences,
    getReferences,
...
export default function ReferencePage() {
    ...
    const [deleteDialogData, setDeleteDialogData] = useState<referencesDialogDataInterface | null>(null);
    ...
    const fetchReferences = async () => {
        ...
        if (data) {
            const formattedData = data.map((r: referencesGetDataInterface) => ({
                ...r,
    ...
    }, [references, keyword, limit]);

    // Delete reference
    const handleDelete = async (data: referencesDialogDataInterface | null) => {
        if (!data) return;
        const res = await deleteReferences(data.id);
        if (res) {
    ...
    const handleEdit = (id?: string) => {
        router.push(`/masters/references/edit/${id}`);
    };
    ...
    const totalTablePages = Math.ceil(filteredReferences.length / rowsPerTablePage);
    const indexOfLastRow = currentTablePage * rowsPerTablePage;
    const indexOfFirstRow = indexOfLastRow - rowsPerTablePage;
    const currentRows = filteredReferences.slice(indexOfFirstRow, indexOfLastRow);
    ...
    return (
        <MasterProtectedRoute>
            ...
            <div className="min-h-[calc(100vh-56px)] overflow-auto max-md:py-10">
                ...
                {/* Delete Dialog */}
                <DeleteDialog<referencesDialogDataInterface>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 relative">
                    <PageHeader title="Dashboard" subtitles={["Reference"]} />
                    {/* Add Button */}

                    <AddButton
                        url="/masters/references/add"
                        text="Add"
