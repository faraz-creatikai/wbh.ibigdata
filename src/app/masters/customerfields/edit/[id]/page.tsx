...
import { getCustomerFieldsById, updateCustomerFields } from "@/store/masters/customerfields/customerfields";

interface ErrorInterface {
  [key: string]: string;
}

export default function CustomerFieldsEdit() {
  const { id } = useParams();
  ...
  // Fetch CustomerFields by ID
  useEffect(() => {
    const fetchCustomerFields = async () => {
      try {
        const data = await getCustomerFieldsById(id as string);
        if (data) setCustomerFieldsData(data);
        else toast.error("CustomerFields not found");
      } catch (error) {
        toast.error("Error fetching customerFields details");
        console.error("Fetch CustomerFields Error:", error);
      } finally {
        setLoading(false);
      }
    };

    if (id) fetchCustomerFields();
  }, [id]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await updateCustomerFields(id as string, customerFieldsData);
      if (result) {
        toast.success("CustomerFields updated successfully!");
        router.push("/masters/customerfields");
      } else toast.error("Failed to update customerFields");
    } catch (error) {
      toast.error("Failed to update customerFields");
      console.error("Update CustomerFields Error:", error);
    }
  };
  ...
  if (loading)
    return (
      <div className="flex justify-center items-center min-h-screen text-gray-600">
        Loading customerFields details...
      </div>
    );
            ...
            <form onSubmit={(e) => e.preventDefault()} className="w-full">
              <div className="mb-8 text-left border-b pb-4 border-gray-200">
                <h1 className="text-3xl font-extrabold text-[var(--color-secondary-darker)] leading-tight tracking-tight">
                  Edit <span className="text-[var(--color-primary)]">CustomerFields</span>
                </h1>
              </div>
