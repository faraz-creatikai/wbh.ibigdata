...
import { customerFieldsDialogDataInterface, customerFieldsGetDataInterface } from "@/store/masters/customerfields/customerfields.interface";
import { deleteCustomerFields, getCustomerFields } from "@/store/masters/customerfields/customerfields";


export default function CustomerFieldsPage() {
    const [customerFields, setCustomerFields] = useState<customerFieldsGetDataInterface[]>([]);
    const [keyword, setKeyword] = useState("");
    const [limit, setLimit] = useState("10");
    const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
    const [deleteDialogData, setDeleteDialogData] = useState<customerFieldsDialogDataInterface | null>(null);
    const [currentTablePage, setCurrentTablePage] = useState(1);
    const [rowsPerTablePage, setRowsPerTablePage] = useState(10);
    const router = useRouter();

    // Fetch customerFields
    const fetchCustomerFields = async () => {
        const data = await getCustomerFields();
        if (data) {
            const formattedData = data.map((r: customerFieldsGetDataInterface) => ({
                ...r,
                Name: r.Name.charAt(0).toUpperCase() + r.Name.slice(1)
            }));
            setCustomerFields(formattedData);
        }
    };
    ...
    // Filtered customerFields
    const filteredCustomerFields = useMemo(() => {
        return customerFields
            .filter((r) => keyword === "" || r.Name.toLowerCase().includes(keyword.toLowerCase()))
    }, [customerFields, keyword, limit]);

    // Delete customerFields
    const handleDelete = async (data: customerFieldsDialogDataInterface | null) => {
        if (!data) return;
        const res = await deleteCustomerFields(data.id);
        if (res) {
            toast.success("CustomerFields deleted successfully!");
            setIsDeleteDialogOpen(false);
            setDeleteDialogData(null);
            fetchCustomerFields();
            return;
        }
        toast.error("Failed to delete customerFields.");
    };

    // Edit customerFields
    const handleEdit = (id?: string) => {
        router.push(`/masters/customerfields/edit/${id}`);
    };
    ...
    const totalTablePages = Math.ceil(filteredCustomerFields.length / rowsPerTablePage);
    const indexOfLastRow = currentTablePage * rowsPerTablePage;
    const indexOfFirstRow = indexOfLastRow - rowsPerTablePage;
    const currentRows = filteredCustomerFields.slice(indexOfFirstRow, indexOfLastRow);
    ...
    return (
        <MasterProtectedRoute>
            ...
            <div className="min-h-[calc(100vh-56px)] overflow-auto max-md:py-10">
                ...
                {/* Delete Dialog */}
                <DeleteDialog<customerFieldsDialogDataInterface>
                    isOpen={isDeleteDialogOpen}
                    title="Are you sure you want to delete this customerFields?"
                ...
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 relative">
                    ...
                    {/* Add Button */}
                    <AddButton
                        url="/masters/customerfields/add"
                        text="Add"
                                ...
                                {currentRows.length > 0 ? (
                                    currentRows.map((r, i) => (
                                        ...
                                        >
                                            <td className="flex items-center gap-10 px-8 py-3 w-1/2">
                                                ...
                                                <p className="w-[200px] font-semibold">{r.Name}</p>
                                            </td>
                                    ...
                                    <tr>
                                        <td colSpan={4} className="text-center py-4 text-gray-500">
                                            No customerFields match your search.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
