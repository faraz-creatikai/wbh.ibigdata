...
import { deleteCity, getCity } from "@/store/masters/city/city";
...
export default function CityPage() {
  ...
  const fetchCities = async () => {
    ...
    const dataFormat = data.map((c: cityAllDataInterface) => ({
      ...c,
  ...
  const currentRows = filteredCities.slice(indexOfFirstRow, indexOfLastRow);
  ...
  const handleEdit = (id?: string) => {
    router.push(`/masters/city/edit/${id}`);
  };
    ...
    <MasterProtectedRoute>
      ...
      <div className="min-h-[calc(100vh-56px)] overflow-auto max-md:py-10">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 relative">
          <PageHeader title="Dashboard" subtitles={["City"]} />
          {/* Add Button */} 
          <AddButton
              ...
              <thead className="bg-[var(--color-primary)] text-white">
                <tr className="flex justify-between items-center w-full">
                  <th className="flex items-center gap-10 px-8 py-3 border border-[var(--color-secondary-dark)] text-left w-1/2">
                    ...
                    <p className="w-[200px] text-left">City Name</p>
                  </th>
