...
import { getCityById, updateCity } from "@/store/masters/city/city";
...
export default function CityEdit() {
  ...
  useEffect(() => {
    const fetchCity = async () => {
      try {
        const data = await getCityById(id as string);
        if (data) {
          setCityData({
            Name: data.Name || "",
            Status: data.Status || "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await updateCity(id as string, cityData);
      if (result) {
          ...
          <form onSubmit={(e) => e.preventDefault()} className="w-full">
            <div className="mb-8 text-left border-b pb-4 border-gray-200">
              <h1 className="text-3xl font-extrabold text-[var(--color-secondary-darker)] leading-tight tracking-tight">
                Edit <span className="text-[var(--color-primary)]">City</span>
              </h1>
            </div>
