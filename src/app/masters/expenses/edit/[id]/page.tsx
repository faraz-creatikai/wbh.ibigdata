...
import { expensesAllDataInterface } from "@/store/masters/expenses/expenses.interface";
...
export default function ExpensesEdit() {
  ...
  const [expensesData, setExpensesData] = useState<expensesAllDataInterface>({
    Name: "",
    Status: "",
  });
  ...
  useEffect(() => {
    const fetchExpense = async () => {
      try {
        const data = await getExpensesById(id as string);
        if (data) setExpensesData(data);
        else toast.error("Expense not found");
      } catch (error) {
  ...
  const handleSelectChange = useCallback((label: string, selected: string) => {
    setExpensesData((prev) => ({ ...prev, [label]: selected }));
    setErrors((prev) => ({ ...prev, [label]: "" }));
  }, []);

  const validateForm = () => {
    const newErrors: ErrorInterface = {};
    if (!expensesData.Name.trim()) newErrors.Name = "Expense Name is required";
    if (!expensesData.Status.trim()) newErrors.Status = "Status is required";
    return newErrors;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await updateExpenses(id as string, expensesData);
      if (result) {
        toast.success("Expense updated successfully!");
        router.push("/masters/expenses");
      } else {
          ...
          <form onSubmit={(e) => e.preventDefault()} className="w-full">
            <div className="mb-8 text-left border-b pb-4 border-gray-200">
              <h1 className="text-3xl font-extrabold text-gray-800 leading-tight tracking-tight">
                Edit <span className="text-[var(--color-primary)]">Expense</span>
              </h1>
            </div>
