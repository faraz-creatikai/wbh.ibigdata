...
import { campaignGetDataInterface, DeleteDialogDataInterface } from "@/store/masters/campaign/campaign.interface";
import { deleteCampaign, getCampaign } from "@/store/masters/campaign/campaign";
...
export default function CampaignPage() {
    const [campaigns, setCampaigns] = useState<campaignGetDataInterface[]>([]);
    ...
    const fetchCampaigns = async () => {
        ...
        /*  const data = [
             {_id:"343", Name: "tesedfasdf;lkj as;dlfkj asdf", Status: "Active" }
         ]; */

        const dataFormat = data.map((c: campaignGetDataInterface) => ({
            ...c,
            Name: c.Name.charAt(0).toUpperCase() + c.Name.slice(1)
        }));

        setCampaigns(dataFormat);
    };
    ...
    const handleDelete = async (data: DeleteDialogDataInterface | null) => {
        if (!data) return;
        const res = await deleteCampaign(data.id);
        if (res) {
    ...
    const handleEdit = (id?: string) => {
        router.push(`/masters/campaign/edit/${id}`);
    };
