...
import {
    contactcampaignGetDataInterface,
    contactcampaignDialogDataInterface
} from "@/store/masters/contactcampaign/contactcampaign.interface";
...
export default function ContactCampaignPage() {
    ...
    const fetchContactCampaigns = async () => {
        const data = await getContactCampaign();
        if (data) {
            const dataFormat = data.map((c: contactcampaignGetDataInterface) => ({
                ...c,
                Name: c.Name.charAt(0).toUpperCase() + c.Name.slice(1)
            }));
            setContactCampaigns(dataFormat);
        }
    };
    ...
    const handleDelete = async (data: contactcampaignDialogDataInterface | null) => {
        if (!data) return;
        const res = await deleteContactCampaign(data.id);
        if (res) {
            toast.success("Contact Campaign deleted successfully!");
            setIsDeleteDialogOpen(false);
            setDeleteDialogData(null);
            fetchContactCampaigns();
            return;
        }
        toast.error("Failed to delete contact campaign.");
    };

    // Edit contact campaign
    const handleEdit = (id?: string) => {
        router.push(`/masters/contact-campaign/edit/${id}`);
    };
    ...
    const totalTablePages = Math.ceil(filteredCampaigns.length / rowsPerTablePage);
        ...
        <MasterProtectedRoute>
            ...
            <div className="min-h-[calc(100vh-56px)] overflow-auto max-md:py-10">
                ...
                <DeleteDialog<contactcampaignDialogDataInterface>
                    ...
                    title="Are you sure you want to delete this contact campaign?"
                                ...
                                {currentRows.length > 0 ? (
                                    ...
                                    <tr>
                                        <td colSpan={4} className="text-center py-4 text-gray-500">
                                            No contact campaigns match your search.
                                        </td>
                                    </tr>
                                )}
                            </tbody>
