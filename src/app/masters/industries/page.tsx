...
import {
    industriesGetDataInterface,
    industriesDialogDataInterface
} from "@/store/masters/industries/industries.interface";

import {
    getIndustries,
    deleteIndustries
} from "@/store/masters/industries/industries"; // make these APIs similar to campaign APIs
import AddButton from "@/app/component/buttons/AddButton";
import PageHeader from "@/app/component/labels/PageHeader";
import MasterProtectedRoute from "@/app/component/MasterProtectedRoutes";

export default function IndustriesPage() {
    const [industries, setIndustries] = useState<industriesGetDataInterface[]>([]);
    ...
    const fetchIndustries = async () => {
        ...
        if (data) {
            const formatted = data.map((c: industriesGetDataInterface) => ({
                ...c,
    ...
    const handleDelete = async (data: industriesDialogDataInterface | null) => {
        if (!data) return;
        const res = await deleteIndustries(data.id);
        if (res) {
    ...
    const handleEdit = (id?: string) => {
        router.push(`/masters/industries/edit/${id}`);
    };
    ...
    const totalTablePages = Math.ceil(filteredIndustries.length / rowsPerTablePage);
        ...
        <MasterProtectedRoute>
            ...
            <div className="min-h-[calc(100vh-56px)] overflow-auto max-md:py-10">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 relative">
                    <PageHeader title="Dashboard" subtitles={["Industries"]} />
                    {/* Add Button */}

                    <AddButton
                        url="/masters/industries/add"
                        text="Add"
