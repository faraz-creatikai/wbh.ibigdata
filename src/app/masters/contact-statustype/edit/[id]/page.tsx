...
export default function ContactStatusTypeEdit() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data: contactstatustypeGetDataInterface | null =
          await getContactStatusTypeById(id as string);

        if (data) {
          setFormData({ Name: data.Name, Status: data.Status });
        } else {
          toast.error("Contact Status Type not found");
        }
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await updateContactStatusType(id as string, formData);

      if (result) {
        toast.success("Contact Status Type updated successfully!");
  ...
  if (loading)
    return (
      <div className="flex justify-center items-center min-h-screen">
        Loading contact status type details...
      </div>
    );
            ...
            <form onSubmit={(e) => e.preventDefault()}>
              <div className="mb-8 border-b pb-4">
                <h1 className="text-3xl font-extrabold">
                  Edit <span className="text-[var(--color-primary)]">Contact Status Type</span>
                </h1>
              </div>
