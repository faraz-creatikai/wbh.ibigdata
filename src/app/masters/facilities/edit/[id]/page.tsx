...
import { getFacilitiesById, updateFacilities } from "@/store/masters/facilities/facilities";
...
export default function FacilitiesEdit() {
  ...
  useEffect(() => {
    const fetchFacilities = async () => {
      try {
        const data = await getFacilitiesById(id as string);
        if (data) {
  ...
  const handleInputChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setFacilitiesData((prev) => ({ ...prev, [name]: value }));
  ...
  const handleSelectChange = useCallback((label: string, selected: string) => {
    setFacilitiesData((prev) => ({ ...prev, [label]: selected }));
  ...
  const validateForm = () => {
    ...
    if (!facilitiesData.Name.trim()) newErrors.Name = "Facility Name is required";
    if (!facilitiesData.Status.trim()) newErrors.Status = "Status is required";
    return newErrors;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await updateFacilities(id as string, facilitiesData);
      if (result) {
        toast.success("Facility updated successfully!");
        router.push("/masters/facilities");
      } else {
