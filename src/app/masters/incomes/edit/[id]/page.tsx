...
import { getIncomeById, updateIncome } from "@/store/masters/income/income";
...
export default function IncomesEdit() {
  ...
  useEffect(() => {
    const fetchIncome = async () => {
      try {
        const data = await getIncomeById(id as string);
        if (data) setIncomeData(data);
        else toast.error("Income not found");
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await updateIncome(id as string, incomeData);
      if (result) {
          ...
          <form onSubmit={(e) => e.preventDefault()} className="w-full">
            <div className="mb-8 text-left border-b pb-4 border-gray-200">
              <h1 className="text-3xl font-extrabold text-gray-800 leading-tight tracking-tight">
                Edit <span className="text-[var(--color-primary)]">Income</span>
              </h1>
            </div>
