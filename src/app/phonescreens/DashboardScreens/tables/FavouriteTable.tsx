...
export default function FavouriteTable<T extends Record<string, any>>({
  ...
  return (
    <div className="w-full space-y-2.5 pb-6">
      ...
      {paginatedLeads.length === 0 && (
        <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/[0.02] py-16">
          <div className="size-12 rounded-full bg-gray-100 dark:bg-white/5 flex items-center justify-center">
            <MdEdit size={20} className="text-gray-400 dark:text-white/25" />
          </div>
      ...
      <AnimatePresence mode="popLayout">
        {paginatedLeads.map((lead, index) => (
          ...
          >
            ...
            <div className="absolute inset-y-0 left-0 w-[3px] rounded-l-2xl bg-[var(--color-primary)] scale-y-0 group-hover:scale-y-100 origin-center transition-transform duration-300" />
            ...
            <div className={[
              ...
              <button
                onClick={() => router.push("/followups/customer")}
                className={[
      ...
      {/* ── Pagination ── */}
      {paginatedLeads.length > 0 && totalPages > 1 && (
        <div className="flex items-center justify-center gap-1.5 pt-3">
          <PageBtn onClick={() => setCurrentPage(1)} disabled={currentPage === 1} title="First">
          ...
          <PageBtn onClick={nextPage} disabled={currentPage === totalPages} title="Next">
            <HiChevronRight size={13} />
          </PageBtn>

          <PageBtn onClick={() => setCurrentPage(totalPages)} disabled={currentPage === totalPages} title="Last">
  ...
  return (
    <button
      ...
      onMouseEnter={(e) => {
        ...
        el.style.backgroundColor = hoverColor ?? "var(--color-primary)";
