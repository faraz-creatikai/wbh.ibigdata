...
export default function ExpenseMarketingAdd() {
  ...
  const validateForm = () => {
    ...
    if (!expenseData.PaymentMethode.trim()) newErrors.PaymentMethode = "Payment Method is required";
    ...
    if (!expenseData.Date.trim()) newErrors.Date = "Date is required";
    return newErrors;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await addExpenseMarketing(expenseData);

      if (result) {
        toast.success("Expense Marketing added successfully!");
        router.push("/financial/expense_marketings");
      } else {
        toast.error("Failed to add Expense Marketing");
      }
    } catch (error) {
      toast.error("Failed to add Expense Marketing");
          ...
          <form onSubmit={(e) => e.preventDefault()} className="w-full">
            ...
            <div className="mb-8 text-left border-b pb-4 border-gray-200">
              <h1 className="text-3xl font-extrabold text-[var(--color-secondary-darker)] leading-tight tracking-tight">
                Add <span className="text-[var(--color-primary)]">Expense Marketing</span>
              </h1>
            </div>
